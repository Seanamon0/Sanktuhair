...
export default async function handler(req, res) {
  ...
  if (imgs && Object.values(imgs).some(Boolean)) {
    ...
    const text = content?.find(b => b.type === 'text')?.text || req.body.description || '';
...
[Liste noire personnalisée et explicite]`
  ...
  // Sauvegarder le diagnostic dans Supabase
  const email = req.body.email || null;
  if (email) {
    await supabase.from('diagnostics').insert({ email, prenom, result });
  }
